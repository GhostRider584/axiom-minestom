...
public enum AxiomPermission {
	...
	private static final NetworkBuffer.Type<AxiomPermission> PERMISSION = NetworkBuffer.STRING.transform(AxiomPermission::getPermissionByName, AxiomPermission::getInternalName);
	public static final NetworkBuffer.Type<EnumSet<AxiomPermission>> PERMISSION_SET = PERMISSION.set().transform(EnumSet::copyOf, EnumSet::copyOf);
	...
	private static AxiomPermission getPermissionByName(String name) {
		for (final var permission : AxiomPermission.values()) {
			if (permission.getInternalName().equals(name)) {
				return permission;
			}
		}
		return null;
